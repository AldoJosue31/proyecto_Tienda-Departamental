...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!user) return failure(401, "UNAUTHENTICATED", "Debes iniciar sesión para registrar una venta.", fallbackCorrelationId);
    if (user.role !== "ADMIN" && user.role !== "EMPLOYEE") {
      return failure(403, "FORBIDDEN", "No tienes permisos para registrar ventas físicas.", fallbackCorrelationId);
    }

    const parsed = requestSchema.safeParse(await request.json().catch(() => null));
    if (!parsed.success) return failure(400, "VALIDATION_ERROR", "Revisa sucursal, cliente y artículos del ticket.", fallbackCorrelationId);

    const key = request.headers.get("Idempotency-Key")?.trim();
    if (!key || !idempotencyKey.test(key)) {
      return failure(400, "INVALID_IDEMPOTENCY_KEY", "La operación no tiene una clave de confirmación válida.", fallbackCorrelationId);
    }
    ...
    const { body, correlationId, response } = await gatewayJson<unknown>("/orders", {
      ...
      body: JSON.stringify({ ...parsed.data, channel: "PHYSICAL" }),
    });
    ...
    if (error instanceof GatewayRequestError) {
      return failure(error.status, "ORDERS_UNAVAILABLE", error.message, error.correlationId);
    }
    return failure(503, "ORDERS_UNAVAILABLE", "No fue posible registrar la venta en este momento.", fallbackCorrelationId);
  }
}
