  ...
  try {
    ...
    if (!user) {
      return Response.json(
        { code: "UNAUTHENTICATED", message: "Debes iniciar sesión para consultar inventario.", correlationId },
        { status: 401, headers: { "X-Correlation-Id": correlationId } },
      );
    }
    if (user.role !== "ADMIN" && user.role !== "EMPLOYEE") {
      return Response.json(
        { code: "FORBIDDEN", message: "No tienes permisos para consultar inventario operativo.", correlationId },
    ...
    const message = error instanceof InventoryDashboardRequestError
      ? error.message
      : "No fue posible consultar Inventory.";
    return Response.json(
      { code: "INVENTORY_UNAVAILABLE", message, correlationId },
