...
import { CrmRequestError, getInactiveCustomers } from "@/lib/crm/crm.server";
...
export async function GET(request: Request) {
  ...
  try {
    ...
    return Response.json(await getInactiveCustomers(months === undefined ? 3 : Number(months)), { headers: { "Cache-Control": "private, no-store", "X-Correlation-Id": correlationId } });
  } catch (error) {
