...
import { CrmRequestError, getCrmCustomer } from "@/lib/crm/crm.server";
...
export async function GET(request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return Response.json(await getCrmCustomer((await params).id), { headers: { "Cache-Control": "private, no-store", "X-Correlation-Id": correlationId } });
  } catch (error) {
