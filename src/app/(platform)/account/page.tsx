  ...
  return (
    <section className="mx-auto max-w-[980px] px-4 py-9 sm:px-6 lg:px-8 lg:py-12">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-[minmax(0,1fr)_minmax(18rem,0.72fr)]">
          ...
          <aside className="rounded-2xl bg-[var(--surface-muted)] p-6"><IconShieldCheck className="text-[var(--accent)]" size={23} aria-hidden="true" /><h2 className="mt-5 font-semibold">Acceso protegido</h2><p className="mt-2 text-sm leading-6 text-[var(--muted)]">Tus permisos se verifican antes de renderizar las rutas privadas y nuevamente en las APIs protegidas.</p>{user.role === "CUSTOMER" ? <div className="mt-7"><div className="flex items-start gap-3"><IconReceipt className="mt-0.5 text-[var(--accent)]" size={19} aria-hidden="true" /><p className="text-sm leading-6 text-[var(--muted)]">Consulta tus compras y el avance de cada entrega desde un único espacio, separado de tus datos de cuenta.</p></div><Link href="/orders" className="mt-4 inline-flex min-h-11 items-center gap-2 rounded-lg px-3 text-sm font-semibold text-[var(--accent-strong)] transition-colors hover:bg-[var(--accent-soft)]">Ver pedidos y entregas</Link></div> : null}</aside>
        </div>
    </section>
  );
}
