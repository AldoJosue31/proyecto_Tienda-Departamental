...
import type { PickPackDashboard, PickPackShipmentDetail, PickPackStatusInput, PickPackTrackingInput } from "./pick-pack-types";
...
const trackingSchema = z.object({
  courier: z.object({ id: z.string().uuid(), name: z.string().min(1) }),
  location: z.object({ latitude: z.number().min(-90).max(90), longitude: z.number().min(-180).max(180), recordedAt: z.string().datetime() }).nullable(),
  locationFreshness: z.enum(["RECENT", "STALE", "UNAVAILABLE"]),
  deliveryAddress: z.string().min(1),
});
const dashboardSchema = z.object({ shipments: z.array(shipmentSchema), refreshedAt: z.string().datetime() });
const detailSchema = z.object({ shipment: shipmentSchema, transitions: z.array(transitionSchema), tracking: trackingSchema.optional() });
...
export function getCustomerShipments(): Promise<PickPackDashboard> { return request("/shipments", dashboardSchema); }
...
export function updatePickPackTracking(id: string, input: PickPackTrackingInput): Promise<PickPackShipmentDetail> { return request(`/shipments/${encodeURIComponent(id)}/tracking`, detailSchema, { method: "PATCH", headers: { "Content-Type": "application/json" }, body: JSON.stringify(input) }); }
