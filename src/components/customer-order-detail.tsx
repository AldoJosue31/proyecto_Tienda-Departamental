...
import { formatCustomerDateTime } from "@/lib/format/customer-date-time";
...
function orderStatusPresentation(status: CustomerOrderStatus) {
  ...
  if (status === "CANCELLATION_PENDING") return { label: "Cancelación en validación", className: "bg-[var(--warning-surface)] text-[var(--warning)]" };
...
export function CustomerOrderDetail({ order, shipment, shipmentError }: CustomerOrderDetailProps) {
  ...
  return <section className="platform-page" aria-labelledby="order-detail-title">
