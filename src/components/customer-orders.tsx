...
export function CustomerOrders({ initialOrders, initialError }: CustomerOrdersProps) {
  ...
  return <section className="mx-auto max-w-[1120px] px-4 py-8 sm:px-6 lg:px-8 lg:py-12" aria-labelledby="customer-orders-title">
    ...
    {isLoading ? <OrdersSkeleton /> : errorMessage && orders.length === 0 ? <Unavailable message={errorMessage} retry={() => { void ordersQuery.refetch(); }} /> : orders.length === 0 ? <EmptyOrders /> : <>
      <div className="mt-8 flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-[var(--muted)]" aria-live="polite">{ordersQuery.isFetching ? "Actualizando pedidos…" : `Mostrando ${orders.length} ${orders.length === 1 ? "pedido" : "pedidos"} registrados`}</p><button type="button" onClick={() => { void ordersQuery.refetch(); }} disabled={ordersQuery.isFetching} className="inline-flex min-h-11 items-center gap-2 rounded-lg px-3 text-sm font-semibold text-[var(--accent-strong)] transition-colors hover:bg-[var(--accent-soft)] disabled:cursor-not-allowed disabled:opacity-45"><IconRefresh size={17} className={ordersQuery.isFetching ? "animate-spin motion-reduce:animate-none" : ""} aria-hidden="true" />Actualizar</button></div>
      {errorMessage ? <div className="mt-4 flex flex-wrap items-start gap-3 rounded-xl border border-[var(--danger)]/30 bg-[var(--danger-surface)] px-4 py-3" role="alert"><IconAlertTriangle className="mt-0.5 shrink-0 text-[var(--danger)]" size={18} aria-hidden="true" /><p className="flex-1 text-sm leading-6">{errorMessage}</p><button type="button" onClick={() => { void ordersQuery.refetch(); }} className="min-h-10 rounded-lg px-2 text-sm font-semibold text-[var(--danger)] underline underline-offset-4">Reintentar</button></div> : null}
      <ol className="mt-5 overflow-hidden rounded-2xl border border-[var(--line)] bg-[var(--surface)] divide-y divide-[var(--line)]">{orders.map((order, index) => <li key={order.id}><OrderRecord order={order} latest={index === 0} /></li>)}</ol>
    </>}
  </section>;
}

function OrderRecord({ order, latest }: { order: CustomerOrder; latest: boolean }) {
