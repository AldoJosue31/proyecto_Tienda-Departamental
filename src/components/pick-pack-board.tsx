...
export function PickPackBoard({ initialDashboard, maps }: { initialDashboard: PickPackDashboard | null; maps: { browserKey: string | null; mapId: string | null } }) {
  ...
  return <section className="min-w-0" aria-labelledby="pick-pack-title" aria-busy={board.isFetching}>
    ...
    {board.error || !data ? <Unavailable message={board.error instanceof Error ? board.error.message : "El tablero no está disponible."} retry={() => void board.refetch()} /> : <>
      ...
      <div className="mt-6 grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_21rem]">
        <div className="min-w-0"><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3" aria-label="Tablero de preparación">{columns.map((column) => <BoardColumn key={column.status} column={column} shipments={data.shipments.filter((shipment) => shipment.status === column.status)} selectedId={selectedId} onSelect={setSelectedId} />)}</div></div>
        <ShipmentDetail shipment={selected} detail={detail.data} loading={detail.isLoading} updating={mutation.isPending} maps={maps} onUpdated={(next) => { queryClient.setQueryData<PickPackDashboard>(["pick-pack-board"], (current) => current ? { ...current, refreshedAt: new Date().toISOString(), shipments: current.shipments.map((shipment) => shipment.id === next.shipment.id ? next.shipment : shipment) } : current); queryClient.setQueryData(["pick-pack-shipment", next.shipment.id], next); }} onAdvance={(shipment, status) => mutation.mutate({ id: shipment.id, input: { status, version: shipment.version } })} />
      </div>
    </>}
  </section>;
}
...
function ShipmentDetail({ shipment, detail, loading, updating, maps, onUpdated, onAdvance }: { shipment: PickPackShipment | null; detail: PickPackShipmentDetail | undefined; loading: boolean; updating: boolean; maps: { browserKey: string | null; mapId: string | null }; onUpdated: (detail: PickPackShipmentDetail) => void; onAdvance: (shipment: PickPackShipment, status: "PACKING" | "SHIPPED" | "DELIVERED") => void }) {
...
function nextStatus(status: PickPackShipment["status"]): "PACKING" | "SHIPPED" | "DELIVERED" | null { return status === "PENDING" ? "PACKING" : status === "PACKING" ? "SHIPPED" : status === "SHIPPED" ? "DELIVERED" : null; }
