...
async function migrationDirectory(): Promise<string> {
  ...
  for (const candidate of candidates) {
    try {
      ...
      // Source and compiled layouts differ. Try the next candidate.
    }
  }
...
async function run(): Promise<void> {
  ...
  try {
    ...
    const appliedResult = await client.query<{ name: string }>(
      "SELECT name FROM inventory_schema_migrations",
    );
    ...
    for (const file of files) {
      ...
      try {
        ...
        await client.query(
          "INSERT INTO inventory_schema_migrations (name) VALUES ($1)",
          [file],
        );
...
run().catch(() => {
  process.stderr.write("Inventory database migration failed.\n");
  process.exitCode = 1;
});
