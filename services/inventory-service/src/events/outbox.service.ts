...
export interface InventoryDomainEvent {
  eventType: "inventory.stock.changed.v1" | "inventory.low-stock.v1";
  correlationId: string | null;
  data: Record<string, unknown>;
}

@Injectable()
export class InventoryOutboxService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(InventoryOutboxService.name);
  private readonly workerId = "inventory-" + randomUUID();
  ...
  async enqueue(client: PoolClient, event: InventoryDomainEvent): Promise<void> {
    await client.query(
      [
        "INSERT INTO inventory_outbox_events (event_type, correlation_id, payload)",
        "VALUES ($1, $2, $3::jsonb)",
      ].join("\n"),
    ...
    try {
      ...
      for (const event of events) {
        try {
          ...
          this.logger.warn("An Inventory outbox event will be retried.");
        }
      }
    } finally {
      this.flushing = false;
  ...
  private async claimPending(): Promise<OutboxRow[]> {
    return this.database.withTransaction(async (client) => {
      const claimed = await client.query<OutboxRow>([
        "WITH candidates AS (",
        "  SELECT id FROM inventory_outbox_events",
        "  WHERE published_at IS NULL AND available_at <= NOW()",
        "    AND (locked_until IS NULL OR locked_until < NOW())",
        "  ORDER BY occurred_at ASC, id ASC",
        "  LIMIT $1 FOR UPDATE SKIP LOCKED",
        ")",
        "UPDATE inventory_outbox_events AS event",
  ...
  private async markPublished(eventId: string): Promise<void> {
    await this.database.query(
      "UPDATE inventory_outbox_events SET published_at = NOW(), locked_by = NULL, locked_until = NULL, last_error = NULL WHERE id = $1 AND locked_by = $2 AND published_at IS NULL",
