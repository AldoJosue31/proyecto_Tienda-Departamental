...
function origins(env: NodeJS.ProcessEnv, environment: string): string[] { const configured = env.CORS_ORIGINS?.split(",").map((value) => value.trim()).filter(Boolean); const result = configured?.length ? configured : ["http://localhost:3000"]; if (result.includes("*")) throw new Error("CORS_ORIGINS must not contain a wildcard."); if (environment === "production" && !configured?.length) throw new Error("CORS_ORIGINS must be explicitly configured in production."); return result; }
function amqp(value: string): string { let parsed: URL; try { parsed = new URL(value); } catch { throw new Error("RABBITMQ_URL must be a valid AMQP URL."); } if (parsed.protocol !== "amqp:" && parsed.protocol !== "amqps:") throw new Error("RABBITMQ_URL must use amqp or amqps."); return parsed.toString(); }
