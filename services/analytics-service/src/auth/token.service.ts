...
import { AUTH_JWT_ISSUER, isRole, type AnalyticsRuntimeConfig, type Role } from "../config/environment";

export const ANALYTICS_RUNTIME_CONFIG = Symbol("ANALYTICS_RUNTIME_CONFIG");
export interface AccessTokenClaims { iss: typeof AUTH_JWT_ISSUER; sub: string; role: Role; exp: number; jti: string; }

@Injectable()
export class TokenService {
  constructor(@Inject(ANALYTICS_RUNTIME_CONFIG) private readonly config: Pick<AnalyticsRuntimeConfig, "accessSecret">) {}
  ...
  private expected(payload: JwtPayload): payload is JwtPayload & AccessTokenClaims {
    return payload.iss === AUTH_JWT_ISSUER && typeof payload.sub === "string" && isRole(payload.role) && typeof payload.exp === "number" && typeof payload.jti === "string";
  }
}
