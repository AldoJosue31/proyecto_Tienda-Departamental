...
export interface OrdersDomainEvent {
  eventType: "order.created.v1" | "order.completed.v1" | "order.cancelled.v1";
  correlationId: string | null;
  data: Record<string, unknown>;
}

@Injectable()
export class OrdersOutboxService implements OnModuleInit, OnModuleDestroy {
  private readonly logger = new Logger(OrdersOutboxService.name);
  private readonly workerId = "orders-" + randomUUID();
  ...
  async enqueue(client: PoolClient, event: OrdersDomainEvent): Promise<void> {
    await client.query(
      [
        "INSERT INTO orders_outbox_events (event_type, correlation_id, payload)",
        "VALUES ($1, $2, $3::jsonb)",
      ].join("\n"),
    ...
    try {
      ...
      for (const event of events) {
        try {
          ...
          this.logger.warn("An Orders outbox event will be retried.");
        }
      }
    } finally {
      this.flushing = false;
  ...
  private async claimPending(): Promise<OutboxRow[]> {
    return this.database.withTransaction(async (client) => {
      const claimed = await client.query<OutboxRow>([
        "WITH candidates AS (",
        "  SELECT id FROM orders_outbox_events",
        "  WHERE published_at IS NULL AND available_at <= NOW()",
        "    AND (locked_until IS NULL OR locked_until < NOW())",
        "  ORDER BY occurred_at ASC, id ASC",
        "  LIMIT $1 FOR UPDATE SKIP LOCKED",
        ")",
        "UPDATE orders_outbox_events AS event",
